...
import { ConfirmDelete, Muted, Pill, ResourceState } from "../components/ui";
...
export function RecipeDetailPage() {
    ...
    const recipe = useResource(() => api.get<AdminRecipeDetail>(`/recipes/${id}`), [id]);
    ...
    const set = <K extends keyof AdminRecipeUpdate>(key: K, next: AdminRecipeUpdate[K]) =>
    ...
    const toggleHidden = async () => {
        ...
        try {
            ...
            recipe.reload();
            toast.show("ok", hiding ? "Hidden from every read path." : "Visible again.");
        } catch (cause) {
            toast.show("error", (cause as Error).message);
        } finally {
            setBusy(undefined);
    ...
    const regenerate = async () => {
        ...
        try {
            const result = await api.post<RegenerateImageResponse>(`/recipes/${data.id}/image`);
                ...
                {data ? (
                    <>
                        <div className="page-head">
                            <div>
                                <h1 className="dish">{data.name}</h1>
                                <p className="page-lede tight">
                                    {data.hiddenAt ? (
                                        <Pill tone="hidden">
                                            Hidden since {formatDateTime(data.hiddenAt)}
                                        </Pill>
                                    ) : (
                                        <Pill tone="live">Live</Pill>
                                    )}{" "}
                                    {data.baseRecipeId ? <Pill tone="info">Version</Pill> : null}{" "}
                                    {data.createdBy ? <Pill tone="rose">Private import</Pill> : null}
                                </p>
                            </div>
                        </div>
                                ...
                                <Paper className="pad">
                                    ...
                                    <Stack spacing={4}>
                                        ...
                                        <TextField
                                            ...
                                            value={
                                                value("shortDescription", data.shortDescription) ?? ""
                                            }
                                            onChange={(event) =>
                                                set("shortDescription", event.target.value || null)
                                            }
                                        ...
                                        <TextField
                                            ...
                                            minRows={4}
                                            label="Description"
                                            value={value("description", data.description) ?? ""}
                                            onChange={(event) =>
                                        ...
                                        <Stack direction="row" spacing={4} flexWrap="wrap" useFlexGap>
                                            <TextField
                                                select
                                                label="Level"
                                                value={value("difficulty", data.difficulty) ?? ""}
                                                onChange={(event) =>
                                                    set(
                                                        "difficulty",
                                                        (event.target.value ||
                                            ...
                                            <TextField
                                                ...
                                                value={value("servings", data.servings) ?? 1}
                                                onChange={(event) =>
                                            ...
                                            <TextField
                                                ...
                                                value={
                                                    value("totalTimeMinutes", data.totalTimeMinutes) ??
                                                    ""
                                                }
                                                onChange={(event) =>
                                                    set(
                                            ...
                                            <TextField
                                                ...
                                                value={
                                                    value("identityCuisine", data.identityCuisine) ?? ""
                                                }
                                                onChange={(event) =>
                                                    set("identityCuisine", event.target.value || null)
                                                }
                                            ...
                                            {data.ingredients.map((ingredient) => (
                                                <TableRow key={ingredient.id}>
                                                    ...
                                                    <TableCell>
                                                        {ingredient.name}
                                                        {ingredient.comment ? (
                                                            <Muted> — {ingredient.comment}</Muted>
                                                        ) : null}
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                            ...
                            <aside>
                                <Paper className="pad">
                                    ...
                                    {data.image ? (
                                        <img
                                            className="hero-art"
                                            src={bustCache(data.image, artToken)}
                                            alt=""
                                        />
                                    ) : (
                                    ...
                                    >
                                        {busy === "image" ? "Painting…" : "Regenerate illustration"}
                                    </Button>
                                    <Muted>
                                        Costs two image generations and takes a few seconds. The
                                ...
                                <Paper className="pad">
                                    ...
                                    <dl>
                                        <div className="kv">
                                            <dt>Favourited</dt>
                                            <dd>{data.references.favourites}</dd>
                                        </div>
                                        <div className="kv">
                                            <dt>In collections</dt>
                                            <dd>{data.references.collections}</dd>
                                        </div>
                                        <div className="kv">
                                            <dt>On shopping lists</dt>
                                            <dd>{data.references.shoppingLists}</dd>
                                        </div>
                                        <div className="kv">
                                            <dt>In menus</dt>
                                            <dd>{data.references.menuCourses}</dd>
                                        </div>
                                        <div className="kv">
                                            <dt>Reader versions</dt>
                                            <dd>{data.references.variants}</dd>
                                        </div>
                                    </dl>
                                </Paper>
                                ...
                                <Paper className="pad">
                                    <h2>Visibility</h2>
                                    <Typography variant="body2" className="card-note">
                                        {data.hiddenAt
                                    ...
                                    {!data.hiddenAt && referenceTotal(data) > 0 ? (
                                        <Alert severity="warning" sx={{ mb: 3 }}>
                                            {referenceTotal(data)} reader reference
                                            {referenceTotal(data) === 1 ? "" : "s"} point at this
                                            dish and will resolve to nothing while it is hidden.
                                        </Alert>
                                    ) : null}
                                    <Button
                                        fullWidth
                                    ...
                                    {data.hiddenReason ? <Muted>Reason: {data.hiddenReason}</Muted> : null}
                                </Paper>
                                ...
                                <Paper className="pad">
                                    ...
                                    <dl>
                                        ...
                                        <div className="kv">
                                            <dt>Canonical id</dt>
                                            <dd className="mono">{data.canonicalId ?? "—"}</dd>
                                        </div>
                                        <div className="kv">
                                            <dt>Id</dt>
                            ...
                            <ConfirmDelete
                                ...
                                warning={
                                    referenceTotal(data) > 0 ? (
                                        <>
                                            It will also remove {referenceTotal(data)} reader
                                            reference{referenceTotal(data) === 1 ? "" : "s"} —
                                            favourites, collections, shopping lists and menu
                                            courses. Hiding it instead keeps all of that.
                                        </>
                                    ) : undefined
                                }
                ...
                <div className="step-row" key={step.id || index}>
                    ...
                    <div>
                        <TextField
                            ...
                            onChange={(event) =>
                                update(index, { instructionText: event.target.value })
                            }
