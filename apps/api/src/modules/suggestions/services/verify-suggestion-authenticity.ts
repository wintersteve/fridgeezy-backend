...
STEP C — Strip any qualifier that adds nothing, on whichever name survived Step B. "Cucumber Sunomono" -> "Sunomono"; "Apple Tarte Tatin" -> "Tarte Tatin". Keep a qualifier ONLY when it marks a genuinely different dish ("Hiroshima-style Okonomiyaki"). A base dish and a redundantly-qualified one are stored as two rows and shown to the user as the same dish twice, which is the single most common defect this step exists to prevent.
  ...
  A CUISINE WORD is the qualifier that adds nothing most often, because the card prints the cuisine directly above the title: "Spicy Thai Cabbage Salad" -> "Spicy Cabbage Salad"; "Chinese Smashed Cucumber Salad" -> "Smashed Cucumber Salad"; "Indian Butter Chicken" -> "Butter Chicken". "Thai Fried Rice" -> "Fried Rice" and "Sichuan Boiled Fish" -> "Boiled Fish" go too: that an English menu prints the origin is NOT a reason to keep it, and neither is another cuisine having a dish by the bare name. Strip it unless the cuisine test in Step A keeps it: part of the name English usage gives the dish (Pad Thai, French Onion Soup, Som Tam Thai), the word that makes a translation name a dish at all ("Vietnamese Spring Rolls"), the only thing separating two real dishes ("Lao Green Papaya Salad"), or part of an INGREDIENT's own name (Chinese broccoli, Thai basil, Swiss chard, Greek yogurt — none of which is the plain version of anything).
  NEVER ADD ONE, and in particular never add one to tell two dishes apart. Two cuisines are allowed to hold the same name here — the catalog keys dish identity on the name AND the cuisine, and separates them itself — so "Manti" stays "Manti" whether it is Turkish or Kazakh. A disambiguating word you add outlives the pair it was added for, and is then printed twice on every card.
