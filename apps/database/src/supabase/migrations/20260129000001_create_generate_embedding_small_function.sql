-- Function to generate OpenAI embeddings using text-embedding-3-small (1536 dimensions)
-- Used for tag embeddings which use the smaller model
create or replace function generate_embedding_small(input_text TEXT)
...
COMMENT on function generate_embedding_small(TEXT) is
'Generates OpenAI text-embedding-3-small vector (1536 dims) for input text. Returns NULL on error. Requires http extension and OpenAI API key. Used for tag embeddings.';
