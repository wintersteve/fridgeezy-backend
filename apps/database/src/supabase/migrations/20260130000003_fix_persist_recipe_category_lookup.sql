-- Fix persist_recipe to look up existing ingredients by canonical_id
-- Never creates categories - ingredients must already exist with their categories
...
begin
    ...
    LOOP
        -- Normalize ingredient name to canonical_id
        v_canonical_id := normalize_to_canonical_id(v_ingredient->>'name');

        -- Look up existing ingredient by canonical_id
        -- Ingredients must be pre-created via match-ingredients service
        ...
        IF v_ingredient_id IS NULL THEN
            RAISE EXCEPTION 'Ingredient "%" (canonical: %) not found. Ingredients must be created through the ingredient matching service before persisting recipes.', v_ingredient->>'name', v_canonical_id;
        END IF;
...
Behavior:
- Looks up ingredients by canonical_id - they MUST already exist
- Ingredients are created via match-ingredients service when suggestion is created
- NEVER creates categories or ingredients - only looks them up
- Raises exception if ingredient not found (suggests data inconsistency)
...
- Creates tags if they dont exist (tags can be created dynamically)
