-- Stop find_recipes returning a suggestion card for a dish it already returned
-- as a recipe card. The guard was there from the start but could never fire:
--
--   and rs.id not in (select id from recipe_rows)
--
-- `recipe_suggestions.id` and `recipes.id` are independent uuid columns in
-- different tables, so that set membership is never true. 15 suggestions matched
-- an existing recipe by name and every one of them rendered as a second card on
-- the search and compose-menu screens (both render recipes + suggestions in one
-- list; the home feed only renders recipes, so it never showed this).
--
-- Compare on the suggestion's own canonical_id instead, applying the exact
-- expression `set_recipe_suggestion_canonical_id()` uses to derive it — matching
-- the stored value's semantics (lower/trim BEFORE the character class, so
-- accents collapse identically) rather than `normalize_to_canonical_id`, which
-- normalizes in a different order and doesn't trim.
--
-- Comparing against `candidate_recipes` (not the whole table) keeps the original
-- intent — suppress what this call already shows. That's also complete in
-- practice: suggestions only reach the output when the recipe matches don't fill
-- limit_count, and in that case candidate_recipes already holds every recipe
