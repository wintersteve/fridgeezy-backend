-- Per-step durations: rename the never-used duration_minutes to duration_seconds
-- and start actually writing it.
--
-- The column has existed since 20260801000007 and has never been written or
-- read — it appears only in the DDL and the generated types. So this rename
-- moves no data and breaks no reader; it is free precisely because the feature
-- was never finished.
--
-- SECONDS, NOT MINUTES. The client works in seconds throughout (cook timers,
-- `parse-durations.ts`), and minutes cannot express the sub-minute steps recipes
-- genuinely have ("cook 30 seconds until fragrant"). An integer-minute column
-- would round those to a minute or, under the positivity constraint, to NULL.
--
-- The check constraint follows the rename automatically — Postgres rewrites a
-- check expression to the new column name — so it still reads
-- "duration_seconds is null or duration_seconds > 0" and needs no restatement.
--
-- Why the model supplies this rather than the client parsing it out of the step
-- text: `step-structure.eval.ts` measures both. The model returns a value on
-- 100% of steps that state a duration, and reads them correctly where a regex
-- cannot — "5 minutes per batch" over three batches is 900s, and the two hours
-- in "After 2 hours braising, continue for 30 minutes" belongs to the previous
-- step. The client parser answers 300s and 9000s respectively.
--
-- Both persist functions are restated in full below because PL/pgSQL bodies can
-- only be replaced wholesale. The only change in either is the instruction
-- INSERT, which now carries duration_seconds.

alter table recipe_instructions
    rename column duration_minutes to duration_seconds;

comment on column recipe_instructions.duration_seconds is
    'How long the step takes or waits, in seconds. Supplied by the generator; null when the step has no meaningful duration.';
...
begin
    ...
    LOOP
        ...
        -- Insert instruction
        INSERT INTO recipe_instructions (recipe_id, step_number, instruction_text, ingredient_refs, duration_seconds)
        VALUES (
            v_recipe_id,
            v_step_number,
            v_instruction->>'text',
