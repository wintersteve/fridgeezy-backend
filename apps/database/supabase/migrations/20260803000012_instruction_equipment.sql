-- Per-step equipment: the tools a step needs, so a card can say "Dutch oven"
-- before the cook starts reading rather than after they have gone looking.
--
-- Unlike duration_seconds and temperature_c, this column does not already
-- exist — those two were declared in 20260801000007 and never written, which is
-- why retyping them was free. This one is a genuine addition.
--
-- text[], NOT a normalised equipment table with references. The values are
-- display-only chips; nothing filters, joins or aggregates on them, and a
-- lookup table would buy referential integrity over a vocabulary no code reads.
-- `recipes.tips` is stored the same way for the same reason. If equipment ever
-- becomes something you search by ("recipes needing no oven"), that is the
-- moment to normalise it — a text[] promotes cleanly, and the reverse does not.
--
-- Values arrive lowercase from the generator and are title-cased at render, the
-- same convention ingredient names already follow.

alter table recipe_instructions
    add column equipment text[];

comment on column recipe_instructions.equipment is
    'Main tools this step needs, lowercase (e.g. {oven,"roasting tin"}). Supplied by the generator; null when the step needs nothing notable.';
