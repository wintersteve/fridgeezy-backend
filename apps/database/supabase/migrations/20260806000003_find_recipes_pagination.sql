-- Offset pagination for find_recipes.
--
-- The client had none: `limit_count` was never passed, so every call took the
-- default 10 and there was no way to reach row 11. A query matching forty
-- recipes showed ten and hid thirty for good — and, because the app read
-- "fewer than 10 came back" as "the catalogue is exhausted, start generating",
-- that same query also never generated. Both halves of the feed were capped by
-- one unpassed argument.
--
-- The old signature is DROPPED rather than replaced. `create or replace` only
-- replaces an identical argument list, so adding `p_offset integer default 0`
-- would leave the five-argument version in place beside the new one — and a
-- five-argument call could then satisfy either, which Postgres rejects at call
-- time as "function is not unique". PostgREST calls by named argument and is no
-- safer here.
