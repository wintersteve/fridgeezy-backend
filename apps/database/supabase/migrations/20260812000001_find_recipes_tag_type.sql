-- The client cannot tell a cuisine tag apart from a dietary/course/dish_form
-- one without `type` — it currently gets only `{id, name}` per tag and would
-- otherwise have to guess from the tag's name string. `recipe_display_tags`
-- and `recipe_suggestion_display_tags` already select `type`; this just stops
-- the aggregate here from dropping it on the floor.
--
-- Signature unchanged, so `create or replace` is enough; no drop is needed.
