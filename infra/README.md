...
The Terraform and the handler (`apps/api/src/lambda.ts`) are both in place.
`/mcp` still needs Phase 2 (session state) before it works on Lambda; the REST
endpoints do not.
...
the MCP handlers read the raw request stream themselves; a real socket satisfies
that for free, where a synthesised `IncomingMessage` would have to reproduce
stream semantics exactly. The cost is a localhost round trip with `TCP_NODELAY`
set on both ends.
...
- The MCP session store (Phase 2) — DynamoDB/ElastiCache, once Phase 2 decides
  whether sessions are needed at all.
